...
if __name__ == '__main__':
    ...
    na = 40
    n_chain = 1
    n_solent = box.x * box.y * box.z * 3 - n_chain * na
    n = na * n_chain + n_solent
    n_bonds = (na - 1) * n_chain
    fcoord = open('COORD', 'w')
    fbonds = open('BONDS', 'w')
    fcoord.write(f'num_atoms {n} box_size {box.x} {box.y} {box.z}\n')
    fbonds.write(
        f'num_bonds {n_bonds} num_atoms {n} box_size {box.x} {box.y} {box.z}\n')
    temp = 0
    for _ in range(n_chain):
        coord = chain(na, box, length_bond=length_bond)
        for i, w in enumerate(coord):
            fcoord.write(
                f'{i+1+temp: <10} {w[0]: <25} {w[1]: <25} {w[2]: <25} {w[3]}\n'.format(*w))
        for i in range(1, na):
